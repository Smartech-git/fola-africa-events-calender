...
const drawLine = (
  ...
  // Draw the whole line so kerning and ligatures match the DOM sizing text.
  const width = measureLine(ctx, line, letterSpacing);
...
const buildTextCanvas = ({
  ...
  const fit = props.fitText
    ? Math.min(1, maxWidth / widest, maxHeight / blockHeight)
    : 1;
