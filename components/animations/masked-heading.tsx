...
const MaskedHeading: React.FC<MaskedHeadingProps> = ({
  ...
  const scaledFontRef = useRef<string | null>(null);
  ...
  const sync = useCallback(() => {
    ...
    // Auto-sizing is opt-in; otherwise CSS controls responsive typography.
    if (s.textScale !== undefined) {
      scaledFontRef.current ??= root.style.fontSize;
      root.style.fontSize = `${clamp(root.clientWidth * s.textScale, 20, 200).toFixed(1)}px`;
    } else if (scaledFontRef.current !== null) {
      root.style.fontSize = scaledFontRef.current;
      scaledFontRef.current = null;
    }
    ...
    for (let i = 0; i < wordRefs.current.length; i += 1) {
      ...
      glyph.style.textTransform = cs.textTransform;
      glyph.style.fontVariant = cs.fontVariant;
    }
    place();
  }, [place]);
